...
import { researchClaimsWithExpansion, openAndExtract, pursueAttestedOrigins, nameFailedStage, PIPELINE_STAGES } from "@/lib/services/web-research-service";
...
async function runReformulation(data: z.infer<typeof schema>, apiKey: string): Promise<ReformOutcome> {
  ...
  if (refreshSources || (hasSource && !activeDossier)) {
    ...
    if (intent) {
      ...
      // ★ الرسمية بالإثبات + طلب الأصل (المادة ٤: الأصل لا الناقل) — في المحرك الواحد
      if (research) activeDossier = await pursueAttestedOrigins(activeDossier, intent, research.findings);
      if (refreshSources) {
  ...
  const systemPrompt = [
    ...
    "",
    "★ تحقق المصطلح النظامي قبل التسليم (إلزامي — إعمالاً للمعيار الثاني «الدقة النظامية والقانونية» من معايير الكتابة المهنية أعلاه في بندَي المصطلحات المعتمدة، ولا يُكتفى فيه بالنية): بعد كتابة النص، أعد قراءته **لفظاً لفظاً** واسأل عن كل لفظ تصف به سلوكاً أو واقعة أو أثراً: هل يَرد في صياغات أنظمة المملكة ولوائحها بهذا المعنى؟ فإن كان لفظاً إعلامياً أو اجتماعياً دارجاً (ولو شاع وفُهم) فأعد صياغة الجملة كاملة بالمصطلح النظامي المكافئ بما يستقيم به المعنى والتذكير والتأنيث والسياق — لا تبديلاً أعمى للكلمة. ولا تُسلّم نصاً فيه لفظ خارج لغة الأنظمة.",
    ...
    WRITING_CODE,
    "",
    "★ التفصيلة التي لا أساس لها لا تُنقل ولا يُبنى عليها (بقرار مالكة المنصة — ملزم): وجودُ تفصيلةٍ في النص الأصلي ليس سنداً لها. فإذا حمل نصُّ المحامي تفصيلةً نظاميةً بلا مصدر — **وأخصُّها التزمينات والمقادير**: مدة، أو مهلة، أو دورية («كل سبعة أيام»، «كل شهر»)، أو رقم، أو نسبة، أو عدد مرات، أو خطوة إجرائية مسمّاة، أو نسبة اختصاص لجهة بعينها — فيحرم عليك أمران معاً: **نقلُها** إلى الصياغة المقترحة، و**تكييفُ الصياغة على مقتضاها** أو البناء عليها أو استكمالها.",
    "عالِجها بأحد وجهين لا ثالث لهما: (أ) التعميم المشروع الذي لا يجزم بتفصيلة («وفق المواعيد المقررة نظاماً»، «بحسب ما تحدده الجهة المختصة»، «بمتابعة دورية منتظمة»)؛ (ب) حذفها كلياً وإعادة بناء الفكرة بدونها متماسكةً إن لم تحمل فائدة. ولا تستبدل بها تفصيلةً أخرى من عندك.",
    "ومن باب أولى: لا تُدخِل في الصياغة معلومةً أو تفصيلةً لم ترد في النص الأصلي أصلاً — فالنص المقترح لا يعلم من الواقعة إلا ما علمه المحامي، ولا يزيد عليه.",
    "",
