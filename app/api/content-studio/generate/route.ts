...
import { needsResearch, researchClaimsWithExpansion, openAndExtract, pursueAttestedOrigins, nameFailedStage, PIPELINE_STAGES } from "@/lib/services/web-research-service";
...
export async function POST(request: Request) {
  ...
  // واللائحة التنفيذية نقطة الانطلاق لكل مسار — تُقرأ أولاً قبل أي كتابة أو حكم
  // أو بحث، والمصادر الخارجية تأتي بعدها وتحتها لا قبلها ولا فوقها.
  //
  // فالمتن الرسمي الكامل أمام الكاتب قبل أن يكتب حرفاً — بقرارها بعد سؤالها
  // المتكرر: «ليش يكتب نص مخالف من الأساس؟». يرى الكاتب ما يراه القاضي:
  // ١٣٧ نصاً حرفياً — ٤٧ قاعدة سلوك و٩٠ مادة لائحة. مخزَّن في المنصة أصلاً،
  // ويُرسل في كتلة system المخزّنة مؤقتاً لدى المزود.
...
قائمة التحقق الختامية (إلزامية قبل الإخراج): راجع نصك مقابل العوامل السبعة واحداً واحداً واسأل عن كل عامل: أين أثره في النص؟ إن غاب أثر أي عامل — القالب لا يطابق النوع، أو الصيغة والطول لا يناسبان القناة وحدّها، أو اللغة لا تناسب الجمهور، أو الرسالة لا تخدم الهدف، أو الموضوع خارج التخصص أو المصدر، أو بقي معيار أو شرط أو تاريخ طلبه المستخدم في خانة الفكرة دون تنفيذ فعلي في النص — فأعد الكتابة قبل الإخراج. وتحقق أخيراً من مستوى الخبير: هل في النص تحليل وتأصيل وزاوية متخصصة، أم هو سرد عمومي يكتبه أي مبتدئ؟ إن كان الثاني فأعد كتابته بعمق الخبير.`;
  ...
  > {
    ...
    // ★ ملف المصادر — المرجع الوحيد (قاعدة المحرك الواحد): يُبنى هنا مرة واحدة
    // من محرك الفهم وباحث الادعاءات، ويُسلَّم مع النسخة فينتقل معها في كل المسارات.
    let dossier: SourceDossier | null = null;
    ...
    // المرجع الأول الذي لا تتجاوزه أي مرحلة. عند الإخفاق تُسمى مرحلته.
    const runUnderstandingAndResearch = async () => {
      // [١] الفهم — كل حقول المستخدم معاً، لا الفكرة وحدها
      const intent = await analyzeIntent({
        topic, specialty, purpose, audience, source, contentType, channel,
      ...
      if (unproven.length > 0) {
        ...
        // ★ الملاحظة تتبع طلب المستخدم (بقرار مالكة المنصة): من لم يطلب مصدراً لا
        // تُعرض له رسالة غياب مصدر أصلاً — كانت تظهر على يوميات وتأمل شخصي لا
        // معلومة نظامية فيهما. ومن طلب مصدراً ولم يُعثر عليه: يُكتب المحتوى،
        // ويُوضَّح له الحال ويُدعى لإرفاق مصدر معتمد وإعادة الإنشاء به.
        sourceNote = proven.length > 0
          ...
          : wantSource
          ? `${pickNoSourceDeclaration(source)} وإن رغبت في تعزيز المحتوى بمصدر، فأرفق مصدراً موثوقاً محدداً من المصادر المعتمدة وأعد إنشاء المحتوى به.`
          : "";
    ...
    // (بقرار المالكة المكرر): «حتى لو ما عنده معلومات يكتب ويوضح» — إشارة الإيقاف
    // الأولى من الكاتب لا تُنهي الطلب: تُقابل بجولة إلزامية للكتابة العامة الصادقة،
    // ولا يقبل الإيقاف إلا إن أصر الكاتب ثانيةً (طلبٌ لا جواب له إلا المعلومة نفسها)
    ...
    for (let attempt = 0; attempt < 4; attempt++) {
      ...
      // ★ إشارة «إيقاف المادة (١٠)» من الكاتب — بقرار المالكة المكرر لا تُقبل من
      // أول مرة: تُقابل بجولة إلزامية للكتابة العامة («يكتب ويوضح أن المحتوى لم
      // يُدعم بمراجع»)، ولا يُقبل الإيقاف إلا إن أصر الكاتب في الجولة الإلزامية
      // (طلب حرفي عن معلومة رسمية بعينها لا جواب له سواها).
      if (raw.some((c) => c === "stop") && !stopRetried) {
        stopRetried = true;
        console.log("[generate:article10] إشارة إيقاف أولى — جولة إلزامية للكتابة العامة (يكتب ويوضح)");
        enforcement.article10Applied = true;
        enforcement.generalAllowed = true;
        if (!sourceNote) sourceNote = pickNoSourceDeclaration(source);
        promptText = `${user}\n\n★ تنبيه ملزم: طلب المستخدم موضوعُ محتوى، لا سؤالاً مباشراً عن معلومة رسمية بعينها — الإيقاف مرفوض. اكتب المحتوى في موضوعه عاماً صادقاً مفيداً بلا أي حكم أو رقم مادة أو مدة أو عقوبة أو نسبة أو أي تفصيلة نظامية (سيُوضَّح للقارئ في ملاحظة مرافقة أن المحتوى لم يُدعَّم بمراجع). لا يجوز سطر الإيقاف إلا إذا كان الطلب حرفياً سؤالاً عن معلومة رسمية محددة (كم العقوبة؟ كم المدة؟ كم الرسم؟) لا جواب له سواها — وعندها فقط أخرج: ${ARTICLE10_STOP_MARKER}`;
        attempt--; // الجولة الإلزامية لا تُحسب من جولات التصحيح
        continue;
      }
    ...
    // ★ بوابة الاكتمال بعد استنفاد الجولات (بقرار المالكة الثابت: التسليم لا
    // الحجب): حكم جوهري مستخرج بقي خارج النص — كما في الصيغ القصيرة (تغريدة
    // بحد ٢٨٠ حرفاً لا تستوعب أحكام مصدر غزير) — يُسلَّم النص مع توثيق النقص:
    // حكم الاكتمال «غير جاهز» يمنع الجاهزية والتوصية حتى المعالجة أو توسيع
    // الصيغة، والمغفل يظهر مسمى في خريطة الاستناد — لا حجب للتسليم.
    if (lastMissingEssential > 0) {
      enforcement.outcome = `${PIPELINE_STAGES[10]}: سُلِّم مع منع الجاهزية — بقي ${lastMissingEssential} حكم جوهري مستخرج خارج النص بعد جولات التصحيح (سعة الصيغة أو طولها)، والنقص موثق في خريطة الاستناد`;
      console.log("[generate:completeness-deliver-blocked-readiness]", enforcement.outcome);
    }
      ...
      try {
        ...
        if (result.kind === "ok") {
          ...
          await completeJob(
            ...
            result.dossier ? JSON.stringify(result.dossier) : undefined,
          );
