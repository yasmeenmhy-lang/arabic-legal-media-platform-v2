...
export async function POST(request: Request) {
  ...
  const sourceFrame = (() => {
    ...
    if (source.includes("عالمية")) return `انطلق من تطور قانوني عالمي واقعي في مجال التخصص المحدد حصراً${source.includes("—") ? ` وبزاوية «${source.split("—")[1].trim()}»` : ""}، واربطه بأثره على القارئ في المملكة. لا تختلق خبراً بتاريخ محدد — صِغه كتطور موثوق منسوب لجهته أو بصيغة «وفق ما نشرته [الجهة]».`;
