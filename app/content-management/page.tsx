...
import { ChevronLeft, ExternalLink, FileClock, Filter, FolderOpen, History, RotateCcw, Trash2 } from "lucide-react";
                  ...
                  <>
                    <tr key={record.id} className="border-b border-line/60 transition hover:bg-paper last:border-none">
                      <td className="px-4 py-4">
                        ...
                        {current && (
                          <p className="mt-0.5 text-xs text-ink/50">{current.contentTypeLabel} · {current.channel}</p>
                        )}
                      </td>
                      <td className="px-4 py-4">
                      ...
                      <td className="px-4 py-4">
                        <div className="flex items-center gap-2">
                          {current && (
                            <Button size="sm" onClick={() => { openVersion(record.id, current.version); }} leadingIcon={<FolderOpen size={14} />}>
                              فتح
                            </Button>
                          )}
                          <button
                    ...
                    {isExpanded && (
                      <tr key={`${record.id}-expanded`} className="bg-paper/60">
