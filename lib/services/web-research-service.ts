...
// ★ آلية الاقتباس إلزامية (بقرار المالكة الأحدث — يحل محل حصر البحث بالمفتاح،
// بعد أن أثبت فحصها الحي أن الكتابة من الذاكرة بلا إسناد تنتج خطأً تاريخياً):
// «حتى لو كان المفتاح مطفأً: إذا استخدم الذكاء أدلة أو مراجع يجب أن تُضاف —
// لحقوق الاقتباس ومصداقية المنصة». فالمفتاح يوجّه البحث بمواصفات المستخدم
// (نوع المصدر/الجهة/الوصف) ولا يُلغي واجب الإسناد: كل نوع محتوى معرفي يمر
// بسلسلة الفهم والإثبات دائماً، وما احتاج إثباتاً ولم يثبت لا يُقدَّم حقيقةً.
export function needsResearch(contentType?: string, _spec?: SourceSpec): boolean {
