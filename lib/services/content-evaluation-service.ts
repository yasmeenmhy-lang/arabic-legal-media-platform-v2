...
- المهنة: أي محتوى يسيء لسمعة المهنة أو يقلل من هيبتها
★ ضابط عند خلوّ المخالفات (بقرار مالكة المنصة — رُصد خرقه حياً): يُمنع أن تجزم أن النص **لا** يعرّض كاتبه للمساءلة أو التبعة أو العقوبة. هذه طمأنةٌ قانونية لا تملكها المنصة، وذكرها نفياً حكمٌ في أمرٍ لم يُطلب منك. قرّر السلامة بما فُحص فقط: لم تُرصد مخالفة لقواعد السلوك أو اللائحة، ولا أثر على الجهات — ثم سمِّ طبيعة النص. ولا نفي مساءلة بعد ذلك. أمّا حين تقع مخالفة فعلاً فوصف أثرها على المحامي يبقى على حاله بلا تحفّظ.
...
المحامي يمثل المهنة في كل ما ينشره. لا تعتمد على كلمات أو أنماط محددة — حكم بذكاء كامل.
...
function parseEvaluationResponse(raw: string): ContentEvaluation {
  ...
  return { risks: enforceNoLiabilityAssurance(enforceStyleOutOfRisks(risks)), professionalWriting, language };
}
...
// ★★ إنفاذ ضابط «لا جزم بنفي المساءلة» (بقرار مالكة المنصة — رُصد حياً على نصّ
// سليم: «...ولا يمس مصلحة موكل أو يعرّض كاتبه للمساءلة»). نفيُ التبعة القانونية
// طمأنةٌ لا تملكها المنصة، ومحلُّ المنع خلوّ المخالفات وحده — فحين تقع مخالفة
// يبقى وصف أثرها على المحامي كما هو. والحذف يقع على الجملة المعروضة فقط: لا
// يُمسّ مستوى الخطر ولا الجهات، فمبناهما المخالفة والضرر لا لفظُ الجملة.
const LIABILITY_DENIAL = /(مساءلة|المساءلة|محاسبة|المحاسبة|عقوبة|العقوبة|تبعة|التبعة|المسؤولية القانونية)/;

function enforceNoLiabilityAssurance(risks: ContentEvaluationRisks): ContentEvaluationRisks {
  if ((risks.affectedParties?.length ?? 0) > 0) return risks;
  const explanation = String(risks.explanation ?? "");
  if (!explanation || !LIABILITY_DENIAL.test(explanation)) return risks;

  // القطع على الفواصل لا الجمل: الخرق يقع شِقّاً داخل جملة واحدة طويلة
  const kept = explanation
    .split(/[،؛]\s*/)
    .filter((part) => part.trim() && !LIABILITY_DENIAL.test(part))
    .join("، ")
    .trim()
    .replace(/[،؛]\s*$/, "");

  console.warn("[evaluation] خرق ضابط نفي المساءلة على نصّ بلا مخالفات — حُذف شقّ الطمأنة");

  return {
    ...risks,
    explanation: kept ? (/[.؟!]$/.test(kept) ? kept : kept + ".") : "لم تُرصد مخالفة لقواعد السلوك المهني أو اللائحة التنفيذية، ولا أثر على الجهات.",
  };
}
